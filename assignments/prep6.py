...
class BoolOp(Expr):
    ...
    def evaluate(self) -> Any:
        ...
        >>> expr.evaluate()
        False
        >>> expr1 = BoolOp('and', [Bool(True), Bool(True), Bool(True)])
        >>> expr2 = BoolOp('and', [Bool(True), expr1, Bool(True)])
        >>> expr2.evaluate()
        True
        """

        if self.op == 'and':
            return all(operand.evaluate() for operand in self.operands)
        elif self.op == 'or':
            return any(operand.evaluate() for operand in self.operands)
        else:
            # We shouldn't reach this branch because of our representation invariant
            raise ValueError(f'Invalid operator {self.op}')
