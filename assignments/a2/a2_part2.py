...
def generate_complete_game_tree(root_move: str, game_state: a2_minichess.MinichessGame,
    ...
    """
    prob_dict = {'White': 1.0, 'Black': 0.0, 'Draw': 0.0, None: 0.0}
    if d == 0 or not game_state.get_valid_moves():
        return a2_game_tree.GameTree(
            root_move, game_state.is_white_move(), prob_dict[game_state.get_winner()])
    else:
        tree = a2_game_tree.GameTree(
            root_move, game_state.is_white_move(), prob_dict[game_state.get_winner()])
        for i in game_state.get_valid_moves():
            tree.add_subtree(
                generate_complete_game_tree(i, game_state.copy_and_make_move(i), d - 1))
        return tree
...
class GreedyTreePlayer(a2_minichess.Player):
    ...
    def make_move(self, game: a2_minichess.MinichessGame, previous_move: Optional[str]) -> str:
        ...
        if self._game_tree is None or self._game_tree.get_subtrees() == []:
            ...
            if previous_move is None and game.is_white_move():
            ...
            elif previous_move is None and not game.is_white_move():
                best_win = min(i.white_win_probability for i in self._game_tree.get_subtrees())
                best_subtree = [i for i in self._game_tree.get_subtrees()
                                if i.white_win_probability == best_win][0]
                self._game_tree = best_subtree
                return best_subtree.move
...
def part2_runner(d: int, n: int, white_greedy: bool) -> None:
    ...
    gametree = generate_complete_game_tree('*', a2_minichess.MinichessGame(), d)
    if white_greedy:
        white = GreedyTreePlayer(gametree)
        black = a2_minichess.RandomPlayer()
        a2_minichess.run_games(n, white, black)
    else:
        white = a2_minichess.RandomPlayer()
        black = GreedyTreePlayer(gametree)
        a2_minichess.run_games(n, white, black)
