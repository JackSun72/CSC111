    ...
    def make_move(self, game: a2_minichess.MinichessGame, previous_move: Optional[str]) -> str:
        ...
        if self._game_tree is None or self._game_tree.get_subtrees() == []:
            ...
            if previous_move is None:
                best_win = max(i.white_win_probability for i in self._game_tree.get_subtrees())
                best_subtree = [i for i in self._game_tree.get_subtrees()
                                if i.white_win_probability == best_win][0]
                random_move = random.choice(game.get_valid_moves())
                random_subtree = self._game_tree.find_subtree_by_move(random_move)
                is_best = random_number >= self._exploration_probability
                map1 = {True: best_subtree, False: random_subtree}
                map2 = {True: best_subtree.move, False: random_move}
                self._game_tree = map1[is_best]
                return map2[is_best]
            else:
...
def run_learning_algorithm(exploration_probabilities: list[float],
    ...
    results_so_far = []
    win_probability_map = {'White': 1.0, 'Black': 0.0, 'Draw': 0.0, None: 0.0}
    # Write your loop here, according to the description above.
    for i in exploration_probabilities:
        white = ExploringPlayer(game_tree, i)
        black = a2_minichess.RandomPlayer()
        winner, sequence = a2_minichess.run_game(white, black)
        game_tree.insert_move_sequence(sequence, win_probability_map[winner])
...
def part3_runner() -> a2_game_tree.GameTree:
    ...
    while i < 1:
        ...
        i += 0.0014
    probabilities.extend([0.0] * 1000)
    # probabilities = [0.4] * 1000
    # probabilities.extend([0.0] * 1000)

    return run_learning_algorithm(probabilities, True)
