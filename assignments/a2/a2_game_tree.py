...
class GameTree:
    ...
    Instance Attributes:
      ...
      - white_win_probability: The current winning probability of white player
    ...
    def __init__(self, move: str = GAME_START_MOVE,
                 is_white_move: bool = True, probability: float = 0.0) -> None:
        ...
        self._subtrees = []
        self.white_win_probability = probability
    ...
    ############################################################################
    def trace(self: GameTree, is_white: bool, reversed_list: list[str], prob: float) -> None:
        """Helps insert_move_sequence function to trace the leaf, insert
         sequence of move and update win probability"""
        if not reversed_list:
            return
        else:
            move = reversed_list.pop()

            subtree = self.find_subtree_by_move(move)
            if subtree:
                current_tree = subtree
                current_tree.trace(not is_white, reversed_list, prob)
            else:
                new_subtree = GameTree(move, is_white, prob)
                self.add_subtree(new_subtree)
                new_subtree.trace(not is_white, reversed_list, prob)
            self._update_white_win_probability()

    def insert_move_sequence(self, moves: list[str], probability: float = 0.0) -> None:
        ...
        move_list = moves[::-1]
        self.trace(False, move_list, probability)
        ...
        if not self._subtrees:
            ...
            if not self.is_white_move:
                self.white_win_probability = \
                    sum(i.white_win_probability for i in self.get_subtrees()) / \
                    len(self.get_subtrees())
            else:
                self.white_win_probability = max(i.white_win_probability
