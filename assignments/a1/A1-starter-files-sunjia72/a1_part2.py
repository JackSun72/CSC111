...
def draw_node(screen: pygame.Surface, node: _Node, pos: Tuple[int, int]) -> None:
    ...
    grid_x = screen_size[0] // GRID_SIZE
    grid_y = screen_size[1] // GRID_SIZE
    width = int(grid_x * 0.4)
    height = int(grid_y * 0.4)

    text = str(node.item)
    x, y = pos[0], pos[1]
    draw_text(screen, text, (int(pos[0] + 0.2 * width), int(pos[1] + 0.4 * height)))
    pygame.draw.line(screen, COLOR, (x, y), (x, y + height))
    pygame.draw.line(screen, COLOR, (x + width, y), (x + width, y + height))
    pygame.draw.line(screen, COLOR, (x + 2 * width, y), (x + 2 * width, y + height))
    pygame.draw.line(screen, COLOR, (x + 2 * width, y + height), (x, y + height))
    pygame.draw.line(screen, COLOR, (x + 2 * width, y), (x, y))
    pygame.draw.circle(screen, COLOR, (x + 1.5 * width, y + 0.5 * height), 3.0)
...
def draw_link(screen: pygame.Surface, start: Tuple[int, int], end: Tuple[int, int]) -> None:
    ...
    pygame.draw.circle(screen, COLOR, start, 3.0)
...
def draw_three_nodes(screen_size: Tuple[int, int]) -> None:
    ...
    draw_node(screen, node1, (initial_x, initial_y))
    draw_link(screen, (initial_x + int(grid_x * 0.4) // 2 * 3, initial_y + height // 2),
              (initial_x + grid_x, initial_y + height // 2))
    draw_node(screen, node2, (initial_x + grid_x, initial_y))
    draw_link(screen, (initial_x + grid_x + int(grid_x * 0.4) // 2 * 3, initial_y + height // 2),
              (initial_x + 2 * grid_x, initial_y + height // 2))
    draw_node(screen, node3, (initial_x + 2 * grid_x, initial_y))
    draw_link(screen, (initial_x + 2 * grid_x + int(grid_x * 0.4) // 2 * 3,
                       initial_y + height // 2),
              (initial_x + 3 * grid_x, initial_y + height // 2))
    draw_text(screen, 'None', (int(initial_x + 3 * grid_x + 0.2 * width),
                               int(initial_y + 0.4 * height)))
...
def draw_list(screen: pygame.Surface, lst: LinkedList, show_grid: bool = False) -> None:
    ...
    while curr is not None:
        draw_node(screen, curr,
                  (initial_x + curr_index % 8 * grid_x,
                   initial_y + curr_index // 8 * grid_y))
        if curr_index % 8 != 7:
            draw_link(screen, (initial_x + curr_index % 8 * grid_x + int(1.5 * width),
                               initial_y + curr_index // 8 * grid_y + int(0.5 * height)),
                      (initial_x + (1 + curr_index) % 8 * grid_x,
                       initial_y + curr_index // 8 * grid_y + int(0.5 * height)))
        else:
            draw_link(screen, (initial_x + curr_index % 8 * grid_x + int(1.5 * width),
                               initial_y + curr_index // 8 * grid_y + int(0.5 * height)),
                      (initial_x + curr_index % 8 * grid_x + int(1.5 * width),
                       (curr_index + 1) // 8 * grid_y - int(0.2 * height)))
            pygame.draw.line(screen, COLOR,
                             (initial_x + curr_index % 8 * grid_x + int(1.5 * width),
                              (curr_index + 1) // 8 * grid_y - int(0.2 * height)),
                             (initial_x + (1 + curr_index) % 8 * grid_x + int(0.5 * width),
                              (curr_index + 1) // 8 * grid_y - int(0.2 * height)))
            pygame.draw.line(screen, COLOR,
                             (initial_x + (1 + curr_index) % 8 * grid_x + int(0.5 * width),
                              (curr_index + 1) // 8 * grid_y - int(0.2 * height)),
                             (initial_x + (1 + curr_index) % 8 * grid_x + int(0.5 * width),
                              initial_y + (curr_index + 1) // 8 * grid_y))
        curr = curr.next
        curr_index = curr_index + 1
    assert curr is None
    draw_text(screen, 'None', (int(initial_x + curr_index % 8 * grid_x + 0.2 * width),
                               int(initial_y + curr_index // 8 * grid_y + 0.4 * height)))
...
def handle_mouse_click(lst: LinkedList, event: pygame.event.Event,
    ...
    Preconditions:
        ...
        - screen_size[1] >= 200
    """
    grid_x_length = screen_size[0] // GRID_SIZE
    grid_y_length = screen_size[1] // GRID_SIZE
    x, y = event.pos[0] // grid_x_length, event.pos[1] // grid_y_length
    index = x + y * GRID_SIZE
    if event.button == 1:
