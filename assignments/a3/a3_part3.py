...
def create_book_graph(review_graph: WeightedGraph,
    ...
    """
    g = WeightedGraph()
    for i in review_graph.get_all_vertices('book'):
        g.add_vertex(i, 'book')
    for i in review_graph.get_all_vertices('book'):
        for j in review_graph.get_all_vertices('book'):
            if i != j:
                weight = review_graph.get_similarity_score(i, j, score_type)
                if weight > threshold:
...
def cross_cluster_weight(book_graph: WeightedGraph, cluster1: set, cluster2: set) -> float:
    ...
    for i in cluster1:
        for j in cluster2:
            sum1 += book_graph.get_weight(i, j)
    return sum1 / (n1 * n2)
