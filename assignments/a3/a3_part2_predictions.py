...
class SimilarUserPredictor(ReviewScorePredictor):
    ...
    def predict_review_score(self, user: str, book: str) -> int:
        ...
        if self.graph.adjacent(user, book):
            ...
            set1 = {(self.graph.get_weight(j, book),
                     self.graph.get_similarity_score(j, user, 'strict'))
                    for j in self.graph.get_neighbours(book)}
            numerator = 0.0
            denominator = 0.0
            for i in set1:
                numerator += i[0] * i[1]
...
def evaluate_predictor(predictor: ReviewScorePredictor,
    ...
    Preconditions:
        ...
        - all users and books in test_file are in predictor.graph
          format described on the assignment handout
    """
    g = a3_part2_recommendations.load_weighted_review_graph(test_file, book_names_file)
    num_reviews = 0
    num_correct = 0
    total_error = 0.0
    for i in g.get_all_vertices('user'):
        for k in g.get_neighbours(i):
            actual_review = g.get_weight(i, k)
            predicted_review = predictor.predict_review_score(i, k)
            if actual_review == predicted_review:
                num_correct += 1
            total_error += abs(actual_review - predicted_review)
            num_reviews += 1
