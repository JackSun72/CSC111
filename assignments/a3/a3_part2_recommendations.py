...
class _WeightedVertex:
    ...
    Same documentation as _Vertex from Part 1, except now bours is a dictionary mapping
    ...
    def similarity_score_unweighted(self, other: _WeightedVertex) -> float:
        ...
        """
        if self.neighbours == set() or other.neighbours == set():
            return 0.0
        else:
            self_neighbours = set(self.neighbours)
            other_neighbours = set(other.neighbours)
            intersection = len(self_neighbours.intersection(other_neighbours))
            union = len(self_neighbours.union(other_neighbours))
    ...
    def similarity_score_strict(self, other: _WeightedVertex) -> float:
        ...
        if self.neighbours == set() or other.neighbours == set():
            ...
            self_neighbours_or = set(self.neighbours)
            other_neighbours_or = set(other.neighbours)
            self_neighbours_and = self_neighbours_or.intersection(other_neighbours_or)
            self_neighbours_and_same = {i for i in self_neighbours_and
                                        if i.neighbours[self] == i.neighbours[other]}
            intersection = len(self_neighbours_and_same)
            union = len(self_neighbours_or.union(other_neighbours_or))
            return intersection / union
...
class WeightedGraph(Graph):
    ...
    def get_similarity_score(self, item1: Any, item2: Any,
        ...
        if score_type == 'unweighted':
            return _WeightedVertex.similarity_score_unweighted(self._vertices[item1],
                                                               self._vertices[item2])
        else:
            return _WeightedVertex.similarity_score_strict(self._vertices[item1],
                                                           self._vertices[item2])
    ...
    def recommend_books(self, book: str, limit: int,
        ...
        *descending order* of similarity score. Ties are broken in descending order
        ...
        Preconditions:
            ...
            - score_type in {'unweighted', 'strict'}
        """
        recommend = list(self.get_all_vertices('book'))
        recommend = sorted(recommend, reverse=True)
        recommend = sorted(recommend, key=lambda x: self.get_similarity_score(
            x, book, score_type), reverse=True)
        recommend.remove(book)
        for i in recommend:
            if self.get_similarity_score(i, book, score_type) == 0:
                recommend.remove(i)
        n = len(recommend)
        if n < limit:
            return recommend
...
def load_weighted_review_graph(reviews_file: str, book_names_file: str) -> WeightedGraph:
    ...
    with open(reviews_file) as csv_file:
        reader = csv.reader(csv_file)
        for row in reader:
            if not row[0] in set(g.get_all_vertices('user')):
                g.add_vertex(row[0], 'user')
            if not dict1[row[1]] in set(g.get_all_vertices('book')):
                g.add_vertex(dict1[row[1]], 'book')
            g.add_edge(dict1[row[1]], row[0], int(row[2]))
    return g
